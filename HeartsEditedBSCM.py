...
class Hearts:
	...
	def __init__(self):  
		self.roundNum = 0
		self.trickNum = 0 # initialization value such that first round is roud 0
		...
		if realPlayer:
			...
			# Make four players
			self.players = [Player("Elek", auto=True), Player("Breanna", auto=True), Player("MarySue", auto=True), MonteCarlo("Monte", auto=True)]
	...
	def playCard(self, player):
		...
		if(self.loops == 20):
			raise Exception("Did not play valid card")
